...
int bacaDataPenduduk(DataPenduduk penduduk[], int maxPenduduk, int* totalPenduduk, int* laki_laki, int* perempuan, int* islam, int* kristen, int* katolik, int* hindu, int* buddha, int* konghucu) {
...
void menu() {
    ...
    do {
        ...
        switch (pilihan) {
            case 1:
                jumlahPenduduk = bacaDataPenduduk(penduduk, MAX_PENDUDUK, &total, &laki_laki, &perempuan, &islam, &kristen, &katolik, &hindu, &buddha, &konghucu);
                if (jumlahPenduduk > 0) {
